...
export default function Recommended({ recommended }) {
  ...
  const handleAddToLove = (e, product) => {
    e.stopPropagation(); // ❗ يمنع التنقل عند الضغط على القلب
    ...
    <div className="container flex flex-col w-full p-4 gap-5">
      ...
      <div className="flex flex-col lg:flex-row gap-3 p-3">
        {recommended.map((el, index) => (
          <div
            key={el.documentId}
            onClick={() => navigate('/books')} // ✅ navigation
            className="container flex justify-between items-center shadow-2xl p-3 rounded cursor-pointer wow animate__animated animate__fadeInUp"
            data-wow-delay={`${0.2 * index}s`}
          >
            <div className="flex gap-3">
              <img
              ...
              <div className="flex flex-col gap-3">
                <h3 className="text-2xl font-bold wow animate__animated animate__fadeInUp">
                  {el.header}
                </h3>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <FaStar
                          key={star}
                          className={`text-xl ${star <= el.rate ? "text-yellow-400" : "text-gray-300"}`}
                        />
                      ))}
                    </div>
